...
#elif BOARD == AUTO_2006
	Driver[0].command.canId = 01;
	Driver[1].command.canId = 02;
	Driver[2].command.canId = 03;
	Driver[3].command.canId = 04;
#else
	Driver[0].command.canId = 5;
	...
	for(int i = 0; i < 8; i++)
	{
	  ...
	  if(Motor[i].type == RM_3508)
		...
		//  Driver[i].unitMode = SPEED_CONTROL_MODE;
			...
			Driver[i].velCtrl.desiredVel[CMD] = 250.0f;
			Driver[i].posCtrl.desiredPos = 0.0f;
			Driver[i].posCtrl.acc = 10.0f*Driver[i].velCtrl.dec;
			Driver[i].posCtrl.posVel = 750.0f;
			Driver[i].homingMode.vel = 160.0f;//
		}
		else
		{
...
void HomingMode(DriverType *driver)
{
	...
	if(fabsf(driver->velCtrl.speed) <=2){		//2&&Times<2
		driver->homingMode.cnt++;
	}else{
		driver->homingMode.cnt = 0;
	}
	
	if(driver->homingMode.cnt >= 800){									//500ms
		driver->homingMode.cnt=0;
		

		if(PosLimType==LOWER_LIMIT_POS)
		{
			//记录下限位零点
			driver->posCtrl.actualPos=0.0f;				
			PosLimit.lowerLimPos=driver->posCtrl.actualPos;
			//向反方向转，检查上限位
			driver->homingMode.vel= -160.f;
			driver->unitMode =HOMING_MODE	;
		}
		//相反方向转到上限位
		if(PosLimType==UPPER_LIMIT_POS)
		{
			PosLimit.upperLimPos=driver->posCtrl.actualPos;
			risePos=PosLimit.upperLimPos+8192;
		...
		Driver->posCtrl.desiredPos = PosLimit.lowerLimPos-8192;
		}
		PosLimType=UPPER_LIMIT_POS;
	}
}
/**
